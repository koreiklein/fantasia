...
from calculus import esmerelda
from ui.render.gl import esmerelda_distances as distances, esmerelda_primitives as primitives, esmerelda_colors as colors
from ui.stack import gl

def render(x, covariant = True):
  if isinstance(x, esmerelda.Variable):
    return renderVariable(x, covariant = covariant)
  elif isinstance(x, esmerelda.Conjunction):
    return renderConjunction(x, covariant = covariant)
  elif x.__class__ == esmerelda.Intersect:
    return renderIntersect(x, covariant = covariant)
  elif x.__class__ == esmerelda.Exists:
    return renderExists(x, covariant = covariant)
  elif x.__class__ == esmerelda.Not:
    return renderNot(x, covariant = covariant)
  elif x.__class__ == esmerelda.Always:
    return renderAlways(x, covariant = covariant)
  elif x.__class__ == esmerelda.AndUnit:
    return renderAndUnit(x, covariant = covariant)
  elif x.__class__ == esmerelda.OrUnit:
    return renderOrUnit(x, covariant = covariant)
  elif isinstance(x, esmerelda.Destructor):
...
def renderVariable(x, covariant = True):
  if x.__class__ == esmerelda.StringVariable:
...
def renderAnd(x, covariant = True):
  if x.right == esmerelda.unit_for_conjunction(x.__class__):
